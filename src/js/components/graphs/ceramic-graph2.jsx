...
var CeramicGraph2 = React.createClass({
    ...
    setupRootNodes: function (data,p) {
        ...
        var heightDistanceBetweenRootNodes = p.innerHeight/rootNodes.length; // height/3/2/2 = height1
        var halfDistanceForMargin = heightDistanceBetweenRootNodes/2;
        var quarterDistanceForMargin = p.innerHeight/40;
    ...
    setupSThemeNodes:function(data,p){
        ...
        xScale.clamp(true);
        // APEP +- r gives us an additional margin of the size of the rects
        var middleRange = [par1.cy * 1.5, par2.cy / 1.5];
        var yScale = d3.scaleLinear().domain([yAxisMultipliers.min,yAxisMultipliers.max]).range(middleRange);
        yScale.clamp(true);

        _.each(sthemeNodes, function (node, i) {
            var measure = p.innerWidth / sthemeNodes.length;
            if(node.name == "Sahbal" || node.name == "MoonVase" || node.name== "TallVase"){

                var ohgohdwhy = node.children;
                _.each(ohgohdwhy,function(n,i) {
                    var parentIndex= _.findIndex(node.parents[0].children,function(c){
                        return c.name === node.name;
                    });
                    var stupidMath = (((i*2)) + (parentIndex * 2)-3)
                    var radian = ((2 * Math.PI)/8 * stupidMath );
                    n.cx = (Math.sin(radian) * p.innerWidth / 10) + node.cx;
                    n.cy = (Math.cos(radian) * p.innerHeight / 10) + node.cy;
                });
                node.color = "#D6F5F1";
            }else{
                node.color = "#E8D969";
